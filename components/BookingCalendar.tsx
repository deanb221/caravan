...
export default function BookingCalendar({ caravan, onDateChange, startDate, endDate }: BookingCalendarProps) {
  ...
  // Check if booking is in March, April, or May
  const isSpringDiscountPeriod = (): boolean => {
    if (!localStartDate) return false;
    const month = localStartDate.getMonth(); // 0-indexed: 2 = March, 3 = April, 4 = May
    return month >= 2 && month <= 4;
  };

  // Calculate total price
  const calculatePrice = (): { basePrice: number; discount: number; finalPrice: number } => {
    ...
    if (days === 3) {
      basePrice = caravan.pricing.weekend;
    }
    // Weekly booking (Friday to Friday = 7 nights)
    else if (days === 7) {
      basePrice = caravan.pricing.weekly;
    }

    // Apply 20% discount for March, April, and May
    const isSpring = isSpringDiscountPeriod();
    const discount = isSpring ? basePrice * 0.2 : 0;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 border border-gray-100">
      <div className="mb-6">
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-lg p-4 mb-4 shadow-md">
          <div className="flex items-center space-x-2">
            <svg className="w-6 h-6 text-green-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <p className="font-bold text-green-900 text-sm mb-1">🌷 Spring Special: 20% Off!</p>
              <p className="text-xs text-green-800">All bookings in March, April & May receive a 20% discount on weekend and weekly rates.</p>
            </div>
          </div>
        </div>
              ...
              <div className="flex justify-between items-center mb-3">
                <div>
                  ...
                  {priceDetails.discount > 0 && (
                    <div className="mb-2">
                      <span className="text-lg text-gray-500 line-through mr-2">£{priceDetails.basePrice.toFixed(2)}</span>
                      <span className="inline-block bg-green-100 text-green-800 text-xs font-semibold px-2 py-1 rounded-full">
                        20% Spring Discount
                      </span>
                    </div>
                  )}
                  <span className="text-4xl font-bold gradient-text">£{totalPrice.toFixed(2)}</span>
                </div>
              </div>
                      ...
                      <div className="text-right">
                        ...
                        {priceDetails.discount > 0 && (
                          <p className="text-xs text-green-600 font-semibold mt-1">
                            Save £{priceDetails.discount.toFixed(2)} (20% off)
                          </p>
                        )}
                      </div>
                    </div>
